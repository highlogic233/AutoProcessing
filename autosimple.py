...
def pre():  # 准备工作。
    if startd == 1:
        return
    global desktop, path, frames_num, fps, width, height, capture, pred
    desktop = get_desktop()
    path = openFile()  # 打开文件。
    if path == '':
        return
...
def getDict():
    global dict, speed1, speed2, speed3, marginTop, marginBottom, marginLeft, marginRight, bias, s1, s2, preserve
    dict = load("setting.npy", allow_pickle=True)
